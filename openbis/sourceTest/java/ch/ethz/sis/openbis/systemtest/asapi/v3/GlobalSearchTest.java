...
//    }
    ...
    @Test
    public void testSearchMatchingExperimentWithSampleProperty()
    {
        ...
        experimentCreation.setCode("EXPERIMENT_SAMPLE_PROPERTY_TEST");
        ...
        sampleCreation.setCode("SAMPLE_SAMPLE_PROPERTY_TEST");
        ...
        dataSetCreation.setCode("DATA_SET_SAMPLE_PROPERTY_TEST");
        ...
        final GlobalSearchCriteria c = new GlobalSearchCriteria();
        // Extra (not existing) words in the search query should not affect the results
        c.withText().thatContains("/CISD/CL1 abra cadabra");

        final GlobalSearchObjectFetchOptions fetchOptions = new GlobalSearchObjectFetchOptions();
        fetchOptions.withMatch();
        final List<GlobalSearchObject> results = search(TEST_USER, c, fetchOptions)
                .getObjects();
        // One original sample should be found and 3 other entities, which refer to that sample as a property.
        assertEquals(results.size(), 4);

        results.forEach(result ->
        {
            final String identifier = result.getObjectIdentifier().toString();
            assertTrue(Arrays.asList("/CISD/CL1", "/CISD/DEFAULT/EXPERIMENT_SAMPLE_PROPERTY_TEST",
                    "/SAMPLE_SAMPLE_PROPERTY_TEST", "DATA_SET_SAMPLE_PROPERTY_TEST").contains(identifier));
            if (!result.getObjectIdentifier().toString().equals("/CISD/CL1"))
            {
                assertEquals(result.getMatch(), "Property 'label': /CISD/CL1");
            } else
            {
                assertEquals(result.getMatch(), "Identifier: /CISD/CL1");
            }
        });
