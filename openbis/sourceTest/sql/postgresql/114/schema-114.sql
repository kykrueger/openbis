...
CREATE DOMAIN data_set_kind AS character varying(40)
	CONSTRAINT data_set_kind_check CHECK (((VALUE)::text = ANY (ARRAY[('EXTERNAL'::character varying)::text, ('LINK'::character varying)::text, ('CONTAINER'::character varying)::text])));
...
CREATE VIEW data AS
    SELECT data_all.id, data_all.code, data_all.dsty_id, data_all.dast_id, data_all.expe_id, data_all.data_producer_code, data_all.production_timestamp, data_all.samp_id, data_all.registration_timestamp, data_all.pers_id_registerer, data_all.pers_id_modifier, data_all.is_placeholder, data_all.is_valid, data_all.modification_timestamp, data_all.is_derived, data_all.ctnr_order, data_all.ctnr_id, data_all.del_id FROM data_all WHERE (data_all.del_id IS NULL);
CREATE VIEW data_deleted AS
    SELECT data_all.id, data_all.code, data_all.dsty_id, data_all.dast_id, data_all.expe_id, data_all.data_producer_code, data_all.production_timestamp, data_all.samp_id, data_all.registration_timestamp, data_all.pers_id_registerer, data_all.pers_id_modifier, data_all.is_placeholder, data_all.is_valid, data_all.modification_timestamp, data_all.is_derived, data_all.ctnr_order, data_all.ctnr_id, data_all.del_id FROM data_all WHERE (data_all.del_id IS NOT NULL);
CREATE SEQUENCE data_id_seq
    ...
    NO MAXVALUE
    CACHE 1;
SELECT pg_catalog.setval('data_id_seq', 25, true);
...
CREATE SEQUENCE data_set_type_id_seq
    ...
    NO MAXVALUE
    CACHE 1;
SELECT pg_catalog.setval('data_set_type_id_seq', 5, true);
...
CREATE TABLE data_set_types (
    ...
    data_set_kind data_set_kind DEFAULT 'EXTERNAL'::character varying NOT NULL
);
...
CREATE SEQUENCE mtpt_id_seq
    ...
    NO MAXVALUE
    CACHE 1;
SELECT pg_catalog.setval('mtpt_id_seq', 24, true);
