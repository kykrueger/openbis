...
CREATE TRIGGER link_data_set_frozen_check_on_delete BEFORE DELETE ON content_copies FOR EACH ROW WHEN (old.data_frozen) EXECUTE PROCEDURE raise_exception_frozen_link_data_set();
CREATE TRIGGER link_data_set_frozen_check_on_insert BEFORE INSERT ON content_copies FOR EACH ROW WHEN (new.data_frozen) EXECUTE PROCEDURE raise_exception_frozen_link_data_set();
