...
CREATE UNIQUE INDEX roas_ag_space_project_bk_uk ON role_assignments USING btree (ag_id_grantee, role_code, (COALESCE((space_id)::bigint, ((-1))::bigint)), (COALESCE((project_id)::bigint, ((-1))::bigint)));
CREATE UNIQUE INDEX roas_pe_space_project_bk_uk ON role_assignments USING btree (pers_id_grantee, role_code, (COALESCE((space_id)::bigint, ((-1))::bigint)), (COALESCE((project_id)::bigint, ((-1))::bigint)));
...
CREATE RULE content_copies_history_insert AS
    ON INSERT TO content_copies DO  INSERT INTO data_set_copies_history (id, cc_id, data_id, external_code, path, git_commit_hash, edms_id, pers_id_author, valid_from_timestamp)
  VALUES (nextval('data_set_copies_history_id_seq'::regclass), new.id, new.data_id, new.external_code, new.path, new.git_commit_hash, new.edms_id, new.pers_id_registerer, new.registration_timestamp);
