...
CREATE VIEW data_set_relationships AS
    SELECT data_set_relationships_all.data_id_parent, data_set_relationships_all.data_id_child, data_set_relationships_all.del_id FROM data_set_relationships_all WHERE (data_set_relationships_all.del_id IS NULL);
...
CREATE VIEW experiments AS
    SELECT experiments_all.id, experiments_all.perm_id, experiments_all.code, experiments_all.exty_id, experiments_all.pers_id_registerer, experiments_all.registration_timestamp, experiments_all.modification_timestamp, experiments_all.proj_id, experiments_all.del_id, experiments_all.is_public FROM experiments_all WHERE (experiments_all.del_id IS NULL);
...
CREATE VIEW sample_relationships AS
    SELECT s.id, s.sample_id_parent, s.relationship_id, s.sample_id_child, s.del_id FROM sample_relationships_all s WHERE (s.del_id IS NULL);
