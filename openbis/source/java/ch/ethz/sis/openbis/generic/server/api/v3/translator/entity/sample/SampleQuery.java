...
public interface SampleQuery extends ObjectQuery
{
    ...
    @Select(sql = "select s.id as objectId, s.saty_id as relatedId from samples s where s.id = any(?{1})", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
    ...
    @Select(sql = "select s.id as objectId, s.samp_id_part_of as relatedId from samples s where s.id = any(?{1})", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
    public List<ObjectRelationRecord> getContainerIds(LongSet sampleIds);

    @Select(sql = "select s.samp_id_part_of as objectId, s.id as relatedId from samples s where s.samp_id_part_of = any(?{1})", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
    public List<ObjectRelationRecord> getContainedIds(LongSet sampleIds);
    ...
    @Select(sql = "select sr.sample_id_parent as objectId, sr.sample_id_child as relatedId from "
            + "sample_relationships sr, relationship_types rt "
            + "where sr.relationship_id = rt.id and rt.code = 'PARENT_CHILD' and sr.sample_id_parent = any(?{1}) order by sr.id", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
    public List<ObjectRelationRecord> getChildIds(LongSet sampleIds);

    @Select(sql = "select s.id as objectId, s.pers_id_registerer as relatedId from samples s where s.id = any(?{1})", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
    public List<ObjectRelationRecord> getRegistratorIds(LongSet sampleIds);

    @Select(sql = "select s.id as objectId, s.pers_id_modifier as relatedId from samples s where s.id = any(?{1})", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
    public List<ObjectRelationRecord> getModifierIds(LongSet sampleIds);

    @Select(sql = "select ma.samp_id as objectId, ma.mepr_id as relatedId from metaproject_assignments ma where ma.samp_id = any(?{1})", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
    public List<ObjectRelationRecord> getTagIds(LongSet sampleIds);

    @Select(sql = "select d.samp_id as objectId, d.id as relatedId from data d where d.samp_id = any(?{1})", parameterBindings = { LongSetMapper.class }, fetchSize = FETCH_SIZE)
