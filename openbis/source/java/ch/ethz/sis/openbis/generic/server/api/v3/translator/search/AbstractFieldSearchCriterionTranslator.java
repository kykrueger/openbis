...
import ch.ethz.sis.openbis.generic.shared.api.v3.dto.search.AbstractFieldSearchCriterion;
import ch.ethz.sis.openbis.generic.shared.api.v3.dto.search.SearchFieldType;
 ...
 * @author pkupczyk
 */
public abstract class AbstractFieldSearchCriterionTranslator extends AbstractSearchCriterionTranslator
{
    ...
    public AbstractFieldSearchCriterionTranslator(IDAOFactory daoFactory, IEntityAttributeProviderFactory entityAttributeProviderFactory)
    {
        this.daoFactory = daoFactory;
        this.entityAttributeProviderFactory = entityAttributeProviderFactory;
    }

    protected DetailedSearchField getDetailedSearchField(SearchTranslationContext context, AbstractFieldSearchCriterion<?> criterion)
    {
        DetailedSearchField field;

        if (criterion.getFieldType().equals(SearchFieldType.PROPERTY))
        {
            field = DetailedSearchField.createPropertyField(criterion.getFieldName());
        } else if (criterion.getFieldType().equals(SearchFieldType.ATTRIBUTE))
        {
        ...
        } else if (criterion.getFieldType().equals(SearchFieldType.ANY_FIELD))
        {
            field = DetailedSearchField.createAnyField(getEntityPropertyTypeDAO(context).listPropertyTypeCodes());
        } else if (criterion.getFieldType().equals(SearchFieldType.ANY_PROPERTY))
        {
            field = DetailedSearchField.createAnyPropertyField(getEntityPropertyTypeDAO(context).listPropertyTypeCodes());
        } else
        {
            throw new IllegalArgumentException("Unknown search field type: " + criterion.getFieldType());
        }
