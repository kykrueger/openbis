...
-- add data set status 
ALTER TABLE external_data ADD COLUMN is_locked BOOLEAN NOT NULL DEFAULT FALSE;
...
ALTER TABLE external_data ADD CONSTRAINT exda_status_enum_ck 
      CHECK (status IN ('ACTIVE', 'ARCHIVED', 'ACTIVATION_IN_PROGRESS', 'ARCHIVIZATION_IN_PROGRESS')); 
-- 