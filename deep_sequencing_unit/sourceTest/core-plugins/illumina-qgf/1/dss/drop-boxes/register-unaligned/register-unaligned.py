...
def process(transaction):
  ...
  print("incomingPath")
  print(incomingPath)
  UNALIGNED_FOLDER=glob.glob(os.path.join(incomingPath, 'Unaligned_[0-9]'))[0]
  #UNALIGNED_FOLDER=incomingPath
  ...
  laneList=glob.glob(UNALIGNED_FOLDER + REGEX_LANES)
  laneList.sort()
  print ("-------------- Lane List --------------")
  print laneList

  undeterminedList=glob.glob(UNALIGNED_FOLDER + REGEX_UNDETERMINED)
  print ("GLOB:")
  print(UNALIGNED_FOLDER + REGEX_UNDETERMINED)
  undeterminedList.sort()
  print ("undeterminedList: ")
  print (undeterminedList)
  ...
  try:
    ...
    print("SHUTIL:")
    print(undeterminedList[0], os.path.join(laneList[0], os.path.basename(undeterminedList[0])))
    shutil.move(undeterminedList[0], os.path.join(laneList[0], os.path.basename(undeterminedList[0])))
  except:
    print ("laneList is not matching the total number of lanes (Missing Project<xxx> folder?)")
  ...
  fileList=glob.glob(UNALIGNED_FOLDER + '/' + REGEX_FILES)
  [fileList.append(i) for i in glob.glob( UNALIGNED_FOLDER +'/' +REGEX_MAKEFILE)]

  basename_folder = os.path.basename(UNALIGNED_FOLDER)
  # Add the incoming file into the data set
  transaction.createNewDirectory(dataSet, basename_folder) 
  # move all files in data set
  [transaction.moveFile(file, dataSet, basename_folder)  for file in fileList]
  print("MOVE:")
  print(os.path.join(UNALIGNED_FOLDER,  BASECALL_STATS_FOLDER + flowcell))
  transaction.moveFile(os.path.join(UNALIGNED_FOLDER,  BASECALL_STATS_FOLDER + flowcell), dataSet, basename_folder)
  ...
  sc = SearchCriteria()
  sc.addMatchClause(SearchCriteria.MatchClause.createAttributeMatch(SearchCriteria.MatchClauseAttribute.CODE, transaction.getIncoming().getName().replace("-", ":")))
  foundSamples = search_service.searchForSamples(sc)
