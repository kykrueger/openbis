...
AFFILIATION= {'FMI': '/links/shared/dsu/dss/customers/fmi/drop-box/','BIOCENTER_BASEL': '/links/shared/dsu/dss/customers/biozentrum/drop-box/', 'NEUROSTEMX': '/links/shared/dsu/dss/customers/biozentrum/drop-box/'}
...
def process(transaction):
  ...
  # expected incoming Name, e.g.: Project_131107_M01761_0043_000000000_A5BNW_1_1
  split=name.split("_")
  if (len(split) == 8):
    runningDate = split[1]
    sequencerId = split[2]
    sequentialNumber = split[3]
    flowCellId = split[4] + "-" + split[5]
    flowLane = "_".join(split[-2:])
    incoming_sample=runningDate+ '_'+ sequencerId + '_' + sequentialNumber + '_' + flowCellId + ':' + flowLane

  # expected Project_130805_M01761_0018_000000000_A59MV_1
  if (len(split) == 7):
    runningDate = split[1]
    sequencerId = split[2]
    sequentialNumber = split[3]
    flowCellId = split[4] + "-" + split[5]
    flowLane = split[-1]
    incoming_sample = '_'.join([runningDate, sequencerId, sequentialNumber, flowCellId]) + ':' + flowLane
  ...
  for f in range(0,len(folders)):
	...
	completeBarcode=barcode + "-" + index2
      ...
      if (parentCode == folderSplit):
        ...
        if len(externalSampleName) > 15:
          usableExternalSampleName = externalSampleName[0:15]
        else:
	...
	extraCopy (affiliation_name, pathToFile) 
      ...
      elif (folderSplit.startswith('lane') and barcode == "NoIndex" and index2 == "NoIndex"):
        usableExternalSampleName = ""
        #print("Creating metadata file for Undetermined:" + nameOfFile)
        # get a file from the IDataSetRegistrationTransaction so it is automatically part of the data set
        #pathToFile = transaction.createNewFile(dataSet, folders[f], nameOfFile)
        #writeMetadataFile(pathToFile, parentPropertyTypes, parentPropertiesMap, fcMetaDataDict, fcMetaDataList, experiment)
        #affiliation_name = parentPropertiesMap[AFFILIATION_PROPERTY_NAME].tryGetAsString()
        #extraCopy (affiliation_name, pathToFile)
        #for foundParent in foundParents:
        ...
        #    writeMetadataFile(fileName, parentPropertyTypes, parentPropertiesMap, fcMetaDataDict, fcMetaDataList, experiment)
        #    affiliation_name = parentPropertiesMap[AFFILIATION_PROPERTY_NAME].tryGetAsString()
    ...
    for file in fastqFileList:
        ...
        if sanitizedExternalSampleName:
          ...
          print ("RENAME file " + file + " TO " + newFile)
          os.rename(file, newFile)
        else:
          # Undetermined file
          newFile = file

        extraCopy (affiliation_name, newFile)
        # finally add the files to the data set     
        transaction.moveFile(newFile , dataSet, folders[f])
