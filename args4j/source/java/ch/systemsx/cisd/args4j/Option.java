 ...
 * <tt>void <i>methodName</i>(T value)</tt>. Its access modified can be anything, but if it's
 * not public, your application needs to run in a security context that allows args4j to access the
 * field/method (see {@link AccessibleObject#setAccessible(boolean)}.
 * <p>
 * The behavior of the annotation differs depending on T --- the type of the field or the parameter
 * of the method.
 * <h2>Boolean Switch</h2>
 * <p>
 * When T is boolean , it represents a boolean option that takes the form of "-OPT". When this
 * option is set, the property will be set to true.
 * <h2>String Switch</h2>
 * <p>
 * When T is {@link String}, it represents an option that takes one operand. The value of the
 * operand is set to the property.
 * <h2>Enum Switch</h2>
 * <p>
 * When T is derived from {@link Enum}, it represents an option that takes an operand, which must
 * be one of the enum constant. The comparion between the operand and the enum constant name is done
 * in a case insensitive fashion.
 * <p>
 * For example, the following definition will represent command line options like "-coin penny" or
 * "-coin DIME" but things like "-coin" or "-coin abc" are errors.
 * 
 * <pre>
 ...
 * <h2>File Switch</h2>
 * <p>
 * When T is a {@link File}, it represents an option that takes a file/directory name as an
 * operand.
 * 
...
public @interface Option
{
    ...
    /**
     * Long name of the option, such as "--foo-long".
     */
     ...
     * <p>
     * At the end of {@link CmdLineParser#parseArgument(String...)}, a {@link CmdLineException}
     * will be thrown if a required option is not present.
     * <p>
    ...
    /**
     * Specify that this option should be skipped for the example command line. This is true even
     * for {@link ExampleMode#ALL}.
     */
    boolean skipForExample() default false;
     ...
     * <p>
     * If this annotation element is used, it overrides the inference and determines the handler to
     * be used. This is convenient for defining a non-standard option parsing semantics.
     * <h3>Example</h3>
     * 
     * <pre>
