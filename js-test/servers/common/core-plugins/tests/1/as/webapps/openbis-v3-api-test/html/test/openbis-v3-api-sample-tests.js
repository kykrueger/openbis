define([ 'jquery', 'openbis', 'test/openbis-v3-api-test-common', 'dto/entity/sample/SampleCreation', 'dto/id/entitytype/EntityTypePermId', 'dto/id/space/SpacePermId', 'dto/id/tag/TagCode' ],
		function($, openbis, c, SampleCreation, EntityTypePermId, SpacePermId, TagCode) {
			return function() {
				 ...
				 * "@type" : "SampleCreation",
				 * 
				 * "typeId" : { "@type" : "EntityTypePermId", "permId" :
				 * "UNKNOWN" },
				 * 
				 * "code" : code,
				 * 
				 ...
				 * 
				 * var ids =
				 * facade.createSamples(creations).then(function(permIds) { var
				 * updates = [ { "@type" : "SampleUpdate",
				 * 
				 * "sampleId" : permIds[0],
				 * 
				 ...
				 * 
				 * return $.when(ids,
				 * createSampleFetchOptions()).then(function(permIds,
				 ...
				 * assertObjectsCount(keys, 1);
				 * 
				 * var sample = samples[keys[0]]; equal(sample.code, code,
				 * "Sample code"); equal(sample.type.code, "UNKNOWN", "Type
				 * code"); equal(sample.space.code, "TEST", "Space code");
				 * start(); }).fail(function(error) { ok(false, error.message);
				 * start(); }); });
				 */
