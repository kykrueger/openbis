define([ 'jquery', 'underscore', 'openbis', 'test/openbis-execute-operations', 'test/common' ], function($, _, openbis, openbisExecuteOperations, common) {
	var executeModule = function(moduleName, openbis) {
		...
		QUnit.test("createSampleTypes()", function(assert) {
			...
			var fCheck = function(sampleType) {
				c.assertEqual(sampleType.getCode(), code, "Sample type code");
				c.assertEqual(sampleType.getPermId().getPermId(), code, "Sample type perm id");
				c.assertEqual(sampleType.getDescription(), "a new description", "Description");
				c.assertEqual(sampleType.isAutoGeneratedCode(), true, "AutoGeneratedCode");
				c.assertEqual(sampleType.isSubcodeUnique(), true, "SubcodeUnique");
				c.assertEqual(sampleType.getGeneratedCodePrefix(), "TEST_PREFIX", "GeneratedCodePrefix");
				c.assertEqual(sampleType.isListable(), true, "Listable");
				c.assertEqual(sampleType.isShowContainer(), true, "ShowContainer");
				c.assertEqual(sampleType.isShowParents(), true, "ShowParents");
				c.assertEqual(sampleType.isShowParentMetadata(), true, "ShowParentMetadata");

				c.assertEqual(sampleType.getPropertyAssignments().length, 1, "Assignments count");

				var assignment = sampleType.getPropertyAssignments()[0];
