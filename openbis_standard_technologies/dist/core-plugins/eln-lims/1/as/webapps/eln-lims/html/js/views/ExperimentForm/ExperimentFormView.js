...
function ExperimentFormView(experimentFormController, experimentFormModel) {
	...
	this.repaint = function(views) {
		...
		var projectCode = Util.getProjectCodeFromExperimentIdentifier(this._experimentFormModel.experiment.identifier);
		var experimentCode = (this._experimentFormModel.mode !== FormMode.CREATE)?Util.getExperimentCodeFromExperimentIdentifier(this._experimentFormModel.experiment.identifier):null;
		...
		if(this._experimentFormModel.mode === FormMode.VIEW) {
			...
			var $uploadBtn = FormUtil.getButtonWithIcon("glyphicon-circle-arrow-up", (function () {
				var space = Util.getSpaceCodeFromIdentifier(_this._experimentFormModel.experiment.identifier);
				var project = Util.getProjectCodeFromExperimentIdentifier(_this._experimentFormModel.experiment.identifier);
				var nameElements = [
					"E",
					space,
					project,
	...
	this._paintIdentificationInfo = function($formColumn) {
		...
		var projectIdentifier = Util.getProjectIdentifierFromExperimentIdentifier(this._experimentFormModel.experiment.identifier);
		...
		if(this._experimentFormModel.mode === FormMode.VIEW || this._experimentFormModel.mode === FormMode.EDIT) {
			...
			$codeField.val(Util.getExperimentCodeFromExperimentIdentifier(this._experimentFormModel.experiment.identifier));
			$codeField.hide();
			...
			$codeField.keyup(function() {
				...
				//Full Identifier
				var currentIdentifierSpace = Util.getSpaceCodeFromIdentifier(_this._experimentFormModel.experiment.identifier);
				var currentIdentifierProject = Util.getProjectCodeFromExperimentIdentifier(_this._experimentFormModel.experiment.identifier);
				var experimentIdentifier = Util.getExperimentIdentifier(currentIdentifierSpace, currentIdentifierProject, _this._experimentFormModel.experiment.code);
			...
			mainController.serverFacade.getProjectFromIdentifier($projectField.val(), function(project) {
				...
				mainController.serverFacade.listExperiments([project], function(data) {
					var autoGeneratedCode = Util.getProjectCodeFromExperimentIdentifier(_this._experimentFormModel.experiment.identifier) + "_EXP_" + (data.result.length + 1);
		...
		for(var j = 0; j < propertyTypeGroup.propertyTypes.length; j++) {
			...
			if(propertyType.code === "XMLCOMMENTS") {
				...
				if(propertyType.code === "SHOW_IN_PROJECT_OVERVIEW") {
					if(!(profile.inventorySpaces.length > 0 && $.inArray(Util.getSpaceCodeFromIdentifier(this._experimentFormModel.experiment.identifier), profile.inventorySpaces) === -1)) {
						continue;
					}
				}
