...
class UserFormParametersRole extends React.PureComponent {
  ...
  handleChange(event) {
    const role = this.getRole(this.props)
    this.props.onChange(UserGroupFormSelectionType.ROLE, {
      id: role.id,
  ...
  handleFocus(event) {
    const role = this.getRole(this.props)
    this.props.onSelectionChange(UserGroupFormSelectionType.ROLE, {
      id: role.id,
  ...
  getRole(props) {
    ...
    if (selection && selection.type === UserGroupFormSelectionType.ROLE) {
