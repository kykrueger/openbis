 ...
 * <p>
 * Subclasses need to implement the method optional_headers(), which returns a list of the optional headers they fill out.
 * <p>
 * Subclasses should implement retrieve_data to get the data they provide.
 * <p>
 * Subclasses should implement add_data_rows. In this method, they should call add_row. The method add_row takes a dictionary as an argument. The keys
 * of the dictionary match the headers in the result columns. The dictionary should include data for the required columns and optional ones they fill.
 * 
 * @author cramakri
 */
public class AbstractRequestHandler implements IRequestHandler
{
     ...
     * 
     * @return The entities parameter or an empty list if no entities were specified.
     */
     ...
     * <li>REFCON : Data that is passed unchanged back to the server when a row is modified. This can be used by the server to encode whatever it
     * needs in order to modify the row. (required)</li>
     ...
     * <li>IMAGES : A map with keys coming from the set 'MARQUEE', 'TILED'. The values are image specs or lists of image specs. Image specs are maps
     * with the keys: 'URL' (a URL for the iamge) or 'DATA'. The data key contains a map that includes the image data and may include some image
