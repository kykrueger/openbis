...
-- Table MS_QUANTIFICATION

CREATE TABLE MS_QUANTIFICATION (
  ID BIGSERIAL NOT NULL,
  EXPE_ID TECH_ID NOT NULL,
  SAMP_ID TECH_ID NOT NULL,
  DS_ID TECH_ID NOT NULL,
  ...
  CONSTRAINT MS_QUANTIFICATION_2 FOREIGN KEY (SAMP_ID) REFERENCES SAMPLES (ID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT MS_QUANTIFICATION_3 FOREIGN KEY (DS_ID) REFERENCES DATA_SETS (ID) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE INDEX MS_QUANTIFICATION_FK_DS_ID_IDX ON MS_QUANTIFICATION(DS_ID);
CREATE INDEX MS_QUANTIFICATION_FK_SAMP_ID_IDX ON MS_QUANTIFICATION(SAMP_ID);
CREATE INDEX MS_QUANTIFICATION_FK_EXPE_ID_IDX ON MS_QUANTIFICATION(EXPE_ID);

-- Table QUANT_CONCENTRATIONS

CREATE TABLE QUANT_CONCENTRATIONS (
  ID BIGSERIAL NOT NULL,
  MS_QUANTIFICATION_ID TECH_ID NOT NULL,
  COMPOUND_ID TECH_ID NOT NULL,
  AMOUNT REAL NOT NULL,
  CALIBRATION_NON_LINEAR BOOLEAN,
  PRIMARY KEY (ID),
  CONSTRAINT FK_QUANT_CONCENTRATIONS_1 FOREIGN KEY (MS_QUANTIFICATION_ID) REFERENCES MS_QUANTIFICATION (ID) ON DELETE CASCADE ON UPDATE CASCADE
);
...
GRANT SELECT ON TABLE MS_QUANTIFICATION TO GROUP metabol_readonly;
GRANT SELECT ON TABLE QUANT_CONCENTRATIONS TO GROUP metabol_readonly;
...
GRANT SELECT ON SEQUENCE MS_QUANTIFICATION_ID_SEQ TO GROUP metabol_readonly;
GRANT SELECT ON SEQUENCE QUANT_CONCENTRATIONS_ID_SEQ TO GROUP metabol_readonly;
...
GRANT ALL PRIVILEGES ON TABLE MS_QUANTIFICATION TO GROUP metabol_readwrite;
GRANT ALL PRIVILEGES ON TABLE QUANT_CONCENTRATIONS TO GROUP metabol_readwrite;
...
GRANT ALL PRIVILEGES ON SEQUENCE MS_QUANTIFICATION_ID_SEQ TO GROUP metabol_readwrite;
GRANT ALL PRIVILEGES ON SEQUENCE QUANT_CONCENTRATIONS_ID_SEQ TO GROUP metabol_readwrite;
